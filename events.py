...
def init_events(db, Location):#this function sets up the events feature as a blueprint
    ...
    def _normalize_positions(event_id):#helper function to fix the positions if they get messed up
        stops = (
            EventStop.query.filter_by(event_id=event_id)
            .order_by(EventStop.position.asc(), EventStop.id.asc())
            .all()
        )
        changed = False
    ...
    def _next_position(event_id):#find position for new stop
        last = (
            EventStop.query.filter_by(event_id=event_id)
            .order_by(EventStop.position.desc())
            .first()
        )
    ...
    def events_list():#shows all community events
        ...
        upcoming = (
            Event.query.filter(Event.event_date >= today, Event.status != "completed")
        ...
        past = (
            Event.query.filter(
                db.or_(Event.event_date < today, Event.status == "completed")
            )
    ...
    def events_create():#function creates an event
        if request.method == "POST":
            ...
            #validate date
            try:
                event_date = dt.datetime.strptime(date_raw, "%Y-%m-%d").date()
            except ValueError:
            ...
            if time_raw:
                try:
                    event_time = dt.datetime.strptime(time_raw, "%H:%M").time()
                except ValueError:
    ...
    def event_detail(event_id):#shows all details of an event
        ...
        _normalize_positions(event.id)

        stops = (
            EventStop.query.filter_by(event_id=event.id)
            .order_by(EventStop.position.asc())
            .all()
        )
    ...
    def event_delete(event_id):#deletes an event
        ...
        # Mark event as completed (creator only)
    ...
    def event_add_location(event_id):#adds a location stop to an event
        ...
        stop = EventStop(
            event_id=event.id,
            location_id=loc.id,
            position=_next_position(event.id),
        )

        db.session.add(stop)
        try:
            db.session.commit()
            flash("Location added to event.", "success")
        except IntegrityError:
            db.session.rollback()
            flash("That location is already in this event.", "info")

        return redirect(url_for("events.event_detail", event_id=event.id))

    # Remove a location from an event (creator only)
    @events_bp.route("/events/<int:event_id>/remove/<int:stop_id>", methods=["POST"])
    @login_required
    def event_remove_stop(event_id, stop_id):#removes a location stop
        event = Event.query.get_or_404(event_id)
        _require_creator(event)

        stop = EventStop.query.get_or_404(stop_id)
        if stop.event_id != event.id:
            abort(400)
    ...
    def event_move_stop(event_id, stop_id):#moves a stop up or down
        ...
        if direction == "up":
            neighbor = EventStop.query.filter_by(event_id=event.id, position=stop.position - 1).first()
        else:
            neighbor = EventStop.query.filter_by(event_id=event.id, position=stop.position + 1).first()
